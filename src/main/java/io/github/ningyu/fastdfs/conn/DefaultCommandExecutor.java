...
import io.github.ningyu.fastdfs.exception.FastDfsException;
import io.github.ningyu.fastdfs.pool.ConnectionPool;
import io.github.ningyu.fastdfs.pool.TrackerLocator;
import io.github.ningyu.fastdfs.protocol.storage.StorageCommand;
import io.github.ningyu.fastdfs.protocol.tracker.TrackerCommand;
    ...
    public <T> T execute(TrackerCommand<T> command) {
        ...
        try {
            ...
            try {
                ...
                } else if(null != conn && broken) {
                    pool.invalidateObject(address, conn);
                }
            } catch (Exception e) {
                logger.error("归还连接到连接池失败", e);
                throw new FastDfsException("归还连接到连接池失败", e);
            }
        }
    }
