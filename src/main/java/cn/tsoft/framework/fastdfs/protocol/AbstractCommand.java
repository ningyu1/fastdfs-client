...
import cn.tsoft.framework.fastdfs.exception.FastDfsIOException;
...
public abstract class AbstractCommand<T> implements BaseCommand<T> {
    ...
    private void send(OutputStream out, Charset charset) throws IOException {
        ...
        logger.debug("发出请求 - 报文头[{}]", request.getHead());
        logger.debug("发出请求 - 报文头[{}], 请求参数[{}]", head, param);
        // 输出报文头
        out.write(head);
        // 输出交易参数
        if (null != param) {
